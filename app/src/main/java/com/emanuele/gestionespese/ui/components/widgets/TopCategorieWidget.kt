...
fun TopCategorieWidget(
    ...
    val displayCount = if (config.heightStep == WidgetHeightStep.S) 3 else config.topN
    val top = remember(spese, displayCount, config.periodo) {
    ...
    WidgetCard(title = "Top categorie", modifier = modifier) {
        if (top.isEmpty()) {
            Text(
                "Nessun dato",
